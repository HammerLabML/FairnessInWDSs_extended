...
if __name__ == '__main__':
    ...
    try:
        with open(os.path.join(os.getcwd(), "networks", "L-Town", "Real", 'dataset_configuration.yaml'), 'r') as f:
    ...
    leakages = pd.read_excel('create_leakage_scenarios_fairness.xlsx', engine='openpyxl', converters={'scenario': int, 'linkid': str})

    start_time = leak_pipes['times']['StartTime']
    end_time = leak_pipes['times']['EndTime']
    #inp_file = leak_pipes['Network']['filename']

    inp_file = os.path.join(os.getcwd(), "networks", "L-Town", "Real", "L-TOWN_v2_Real.inp")  
    results_folder = os.path.join(os.getcwd(), "networks", "L-Town", "Real", "LeakageScenarios")
    ...
    class LeakDatasetCreator:
        ...
        def dataset_generator(self):
            ...
            if number_of_leaks:
                leakages_folder = os.path.join(results_folder, f"Scenario-{str(self.scenario)}", "Leakages")
            ...
            if self.results:
                ...
                for leak_i in range(0, len(leak_node)):
                    ...
                    writer = pd.ExcelWriter(os.path.join(f'{leakages_folder}', f'Leak_{NODEID}.xlsx'), engine='xlsxwriter')
                ...
                if not os.path.isdir(os.path.join(results_folder, f'Scenario-{str(self.scenario)}', "Measurements_All")):
                    os.system('mkdir ' + os.path.join(results_folder, f'Scenario-{str(self.scenario)}', "Measurements_All"))
                writer = pd.ExcelWriter(os.path.join(results_folder, f'Scenario-{str(self.scenario)}', "Measurements_All", 'Measurements_All.xlsx'), engine='xlsxwriter')
    ...
    def generate_leaky_scenario(scenario_id):
        if isnan(scenario_id):
            return
        L = LeakDatasetCreator(scenario_id)
        L.dataset_generator()

    Parallel(n_jobs=1)(delayed(generate_leaky_scenario)(s_id) for s_id in leakages['scenario'])
    ...
    #os.startfile(logfilename)  # Not possible on Linux/Unix systems