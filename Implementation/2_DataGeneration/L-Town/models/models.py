...
"""
    Graph convolution based model using multiple linear and GCN layers and Residual Latent Connections
"""
class GCN_Predictor_Multi(torch.nn.Module):
    def __init__(self, in_channels, out_channels, edge_channels, hidden_channels=96, n_aggr=45, n_hops=1, bias=False, num_layers=2, dropout=0., batch_size=48):
        super(GCN_Predictor_Multi, self).__init__()
        self.n_aggr = n_aggr
        self.n_hops = n_hops
        self.batch_size = batch_size
        self.hidden = hidden_channels
        self.out_channels = out_channels
        self.node_in = torch.nn.Linear(in_channels, hidden_channels, bias=bias)        
        self.node_out = torch.nn.Linear(hidden_channels, out_channels, bias=bias)
        self.edge = torch.nn.Linear(edge_channels, hidden_channels, bias=bias)        
            
        self.gcn_aggrs = torch.nn.ModuleList()
        for _ in range(n_aggr):
            gcn = GENConvolution(hidden_channels, hidden_channels, hidden_channels, aggr="add", bias=bias, num_layers=num_layers, dropout=dropout)
            self.gcn_aggrs.append(gcn)
